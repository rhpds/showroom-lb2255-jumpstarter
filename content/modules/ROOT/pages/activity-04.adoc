...
When a Lease is created, the Jumpstarter Service attempts to find a matching Exporter using a label selector. Once a Lease has been aquired, 
...
A Lease is a way to access an Exporter. To selecte an Exporter of `type=virtual` and create a Lease, run the following command:
